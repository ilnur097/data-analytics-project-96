...
all_keys AS (
    SELECT visit_day AS report_date, source, medium, campaign FROM aggregated_sessions
    UNION
    SELECT ad_date AS report_date, source, medium, campaign FROM aggregated_costs
    UNION
    SELECT visit_day AS report_date, source, medium, campaign FROM aggregated_leads
)
SELECT 
